import streamlit as stimport numpy as npimport pandas as pdimport cvxpy as cpimport plotly.graph_objects as goimport plotly.express as pximport yfinance as yf
...
st.markdown("""<style>.stApp { background-color: #0b0f19; }h1, h2, h3, h4 { color: #63b3ed; font-family: 'Inter', sans-serif; }.metric-container { background-color: #1a202c; padding: 20px; border-radius: 10px; border-left: 4px solid #63b3ed; box-shadow: 0 4px 6px rgba(0,0,0,0.3); }.metric-label { color: #a0aec0; font-size: 14px; text-transform: uppercase; letter-spacing: 1px; }.metric-value { color: #ffffff; font-size: 28px; font-weight: bold; }.metric-sub { color: #fc8181; font-size: 14px; }.audit-log { font-family: 'Courier New', monospace; background-color: #000000; color: #4ade80; padding: 15px; border-radius: 5px; font-size: 13px; }</style>""", unsafe_allow_html=True)

st.title("📈 Institutional Tax-Aware Portfolio Terminal")st.markdown("Powered by CVXPY Convex Optimization to map macro tax impacts on the Markowitz Frontier.")st.divider()

--- LIVE MARKET DATA ENGINE ---

assets = ['Equities (SPY)', 'Fixed Income (AGG)', 'Real Estate (VNQ)', 'Commodities (GLD)']num_assets = len(assets)

@st.cache_datadef load_live_market_data():"""Pulls 10 years of live market data to calculate real empirical covariance and expected returns."""tickers = ["SPY", "AGG", "VNQ", "GLD"]
...
with st.spinner("Downloading 10-year empirical market data..."):expected_returns, cov_matrix = load_live_market_data()

--- CVXPY CONVEX MATH ENGINE ---

@st.cache_datadef generate_efficient_frontier(returns, cov_mat, min_weight, max_weight):"""Uses CVXPY to rigorously map the efficient frontier via quadratic programming."""w = cp.Variable(num_assets)
...
with st.sidebar:st.header("⚙️ Terminal Controls")
...
tax_vector = np.array([tax_rates['equity_tax'], tax_rates['bond_tax'], tax_rates['real_estate_tax'], tax_rates['commodity_tax']])post_tax_returns = expected_returns * (1 - tax_vector)

vols_pre, rets_pre, weights_pre = generate_efficient_frontier(expected_returns, cov_matrix, min_w, max_w)vols_post, rets_post, weights_post = generate_efficient_frontier(post_tax_returns, cov_matrix, min_w, max_w)

Find the portfolio that best matches the user's selected volatility

if len(vols_pre) > 0 and len(vols_post) > 0:idx_pre = (np.abs(vols_pre - target_vol)).argmin()idx_post = (np.abs(vols_post - target_vol)).argmin()else:st.error("Algorithm failed to converge. The parameters provided result in an infeasible mathematical space.")st.stop()

--- TOP METRICS ---

m1, m2, m3 = st.columns(3)

drag = (np.mean(rets_pre) - np.mean(rets_post)) * 100turnover_penalty = np.sum(np.abs(weights_pre[idx_pre] - weights_post[idx_post])) * 0.005 * 100 # Simulated 50bps transaction cost

m1.markdown(f"<div class='metric-container'><div class='metric-label'>Active Scenario</div><div class='metric-value' style='font-size:18px; margin-top:10px;'>{scenario}</div></div>", unsafe_allow_html=True)m2.markdown(f"<div class='metric-container'><div class='metric-label'>Mean Frontier Compression</div><div class='metric-value' style='color:#fc8181;'>-{drag:.2f}%</div><div class='metric-sub'>Yield drag across curve</div></div>", unsafe_allow_html=True)m3.markdown(f"<div class='metric-container'><div class='metric-label'>Regime Transition Friction</div><div class='metric-value' style='color:#f6e05e;'>{turnover_penalty:.2f}%</div><div class='metric-sub'>Estimated transaction costs</div></div>", unsafe_allow_html=True)
...
with col1:st.subheader("Interactive Efficient Frontier")
...
with col2:st.subheader(f"Allocation at Selected Risk")
...
st.divider()st.subheader("🧾 NLP Data Pipeline Audit Log")st.markdown("Transparent ledger tracking the translation of natural language policy into hard mathematical CVXPY constraints.")

audit_text = f"""[SYSTEM INITIALIZATION] Target: Convex Optimization Engine (CVXPY)[STATUS] Extracting unstructured parameters...[MATCH] Scenario Context: '{scenario}'[DATA] Vectorizing parsed tax regulations:-> Equities Target Limit     : {tax_rates['equity_tax']*100}%-> Fixed Income Target Limit : {tax_rates['bond_tax']*100}%-> Real Estate Target Limit  : {tax_rates['real_estate_tax']100}%-> Commodities Target Limit  : {tax_rates['commodity_tax']100}%[COMPUTATION] Recalibrating return vectors (R_post = R_pre * (1 - T_asset))...[CONSTRAINTS] Calculating mathematically feasible bounds: Min {min_w100}%, Max {max_w100}%[EXECUTION] Automated OSQP/CLARABEL Solver Engaged. Frontier Mapping Complete."""
