...
This repository demonstrates an experimental architecture that bridges **Natural Language Processing (LLMs)** and **Operations Research (Mean-Variance Optimization)**. By parsing hypothetical tax regulations into structured JSON parameters, the engine mathematically penalizes expected returns to optimize for the maximum *after-tax* Sharpe Ratio.

### ⚙️ Methodology Pipeline
1. **Unstructured Input:** Accepts raw text outlining hypothetical or proposed tax regulations.
2. **LLM Parameter Extraction:** Simulates an LLM parsing the text to extract specific tax rates for Equities, Bonds, Real Estate, and Commodities.
...
* **NLP Parsing (Architecture):** OpenAI/LangChain (Simulated for PoC)
